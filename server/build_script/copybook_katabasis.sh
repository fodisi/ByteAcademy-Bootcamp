...
#                                                                              #
# Be advised:                                                                  #
# ~ odisi must be a string that is not "root"                          #
# ~ Fodisi012823. should be a string that is longer than eight characters      #
...
# Assuming that we make the droplet with the dashboard or a series of curl
...
# Adds user odisi to the sudo group
...
# 'enable' makes a program restart automatically after a reboot.
systemctl enable firewalld

#setting a random default port to be used by the firewall
...
# installs network time protocol
...
sh -c 'echo "destemail = francis2301@gmail.com" >> /etc/fail2ban/jail.local'

sh -c 'echo "sendername = security@digitalocean-ubuntu-fodisi" >> /etc/fail2ban/jail.local'
...
# CREATE USER odisi WITH PASSWORD 'Fodisi012823.';

# CREATE DATABASE master OWNER odisi;
